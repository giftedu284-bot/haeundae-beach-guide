...
const beachCellsByRow = { A: [4, 12], B: [2, 12], C: [1, 12], D: [1, 12], E: [1, 11], F: [2, 10], G: [4, 9] };
const beachBoundary = [
  [35.1592391,129.1622327],[35.1592232,129.1623125],[35.1592248,129.1624084],[35.1592578,129.1625103],[35.1593367,129.1626069],[35.1593937,129.1626873],[35.1594990,129.1634920],[35.1596393,129.1647151],[35.1595604,129.1679445],[35.1595284,129.1684263],[35.1592851,129.1696193],[35.1591295,129.1698984],[35.1586275,129.1692216],[35.1587919,129.1690404],[35.1589040,129.1685307],[35.1588973,129.1677853],[35.1588778,129.1663901],[35.1587310,129.1653382],[35.1585189,129.1631374],[35.1581019,129.1610843],[35.1576022,129.1590037],[35.1569709,129.1571193],[35.1564239,129.1557987],[35.1558795,129.1549044],[35.1556784,129.1547728],[35.1556554,129.1546597],[35.1557847,129.1545308],[35.1562062,129.1543963],[35.1563277,129.1544186],[35.1565279,129.1544317],[35.1567272,129.1547480],[35.1570517,129.1550270],[35.1572008,129.1552201],[35.1579596,129.1569151],[35.1584025,129.1579452],[35.1586095,129.1586556],[35.1587411,129.1593747],[35.1591353,129.1614219],[35.1592743,129.1621183]
];
...
const geoFacilities = [
  ...
  { lat: 35.1599499598263, lng: 129.167356608346, label: "🦶", title: "세족장", detail: "세족 시설", group: "hygiene" },
  { lat: 35.1590453706356, lng: 129.159857942827, label: "🦶", title: "세족장", detail: "세족 시설", group: "hygiene" },
  { lat: 35.15856973636371, lng: 129.15780122463147, label: "🦶", title: "세족장", detail: "세족 시설", group: "hygiene" },
  { lat: 35.15852700538682, lng: 129.15792354069478, label: "🚿", title: "샤워장", detail: "샤워 시설", group: "hygiene" },
  { lat: 35.15762492509603, lng: 129.15582585380992, label: "🦶", title: "세족장", detail: "세족 시설", group: "hygiene" },
  { lat: 35.15761273866907, lng: 129.15587765281612, label: "🚿", title: "샤워장", detail: "샤워 시설", group: "hygiene" },
...
function isBeachCell(row, column) { const [first, last] = beachCellsByRow[row]; return column >= first && column <= last; }
function letterLabel(index) { let value = index + 1, label = ""; while (value > 0) { value--; label = String.fromCharCode(65 + (value % 26)) + label; value = Math.floor(value / 26); } return label; }
function isInsideBeach(lat, lng) { let inside = false; for (let i = 0, j = beachBoundary.length - 1; i < beachBoundary.length; j = i++) { const [latI, lngI] = beachBoundary[i], [latJ, lngJ] = beachBoundary[j]; const crosses = (latI > lat) !== (latJ > lat) && lng < ((lngJ - lngI) * (lat - latI)) / (latJ - latI) + lngI; if (crosses) inside = !inside; } return inside; }
...
function toLocal(lat, lng) { return { u: (lng - gridOrigin.lng) * metresPerLng, v: (lat - gridOrigin.lat) * metresPerLat }; }
function toLatLng(u, v) { return { lat: gridOrigin.lat + v / metresPerLat, lng: gridOrigin.lng + u / metresPerLng }; }
function pointInPolygon(point, polygon) { let inside = false; for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) { const a = polygon[i], b = polygon[j]; if ((a.v > point.v) !== (b.v > point.v) && point.u < ((b.u - a.u) * (point.v - a.v)) / (b.v - a.v) + a.u) inside = !inside; } return inside; }
function segmentsCross(a, b, c, d) { const cross = (p, q, r) => (q.u - p.u) * (r.v - p.v) - (q.v - p.v) * (r.u - p.u); const ab1 = cross(a, b, c), ab2 = cross(a, b, d), cd1 = cross(c, d, a), cd2 = cross(c, d, b); return ab1 * ab2 < 0 && cd1 * cd2 < 0; }
function fullCellIsInBeach(cellPath, localBoundary) { if (!cellPath.every((point) => pointInPolygon(point, localBoundary))) return false; const centre = { u: (cellPath[0].u + cellPath[2].u) / 2, v: (cellPath[0].v + cellPath[2].v) / 2 }; if (!pointInPolygon(centre, localBoundary)) return false; for (let side = 0; side < 4; side++) { const start = cellPath[side], end = cellPath[(side + 1) % 4]; for (let edge = 0; edge < localBoundary.length; edge++) if (segmentsCross(start, end, localBoundary[edge], localBoundary[(edge + 1) % localBoundary.length])) return false; } return true; }
function selectGeoCell(address, polygon) { if (selectedGeoCell) selectedGeoCell.setOptions({ fillOpacity: 0.08, strokeColor: "#237a8b" }); selectedGeoCell = polygon; selectedGeoCell.setOptions({ fillOpacity: 0.52, fillColor: "#f6b73c", strokeColor: "#d76b00" }); selectAddress(address); }
function findGeoCell(lat, lng) { if (!isInsideBeach(lat, lng)) return null; const point = toLocal(lat, lng); for (const [address, cell] of geoGrid) if (pointInPolygon(point, cell.localPath)) return { address, cell }; return null; }
function clearSearchOverlays() { searchOverlays.forEach((overlay) => overlay.setMap(null)); searchOverlays = []; }
function renderSearchOverlays() { if (!kakaoMap || !window.kakao || !window.kakao.maps) return; clearSearchOverlays(); reports.forEach((report) => { const cell = geoGrid.get(report.grid); if (!cell) return; const path = cell.localPath.map(({ u, v }) => { const { lat, lng } = toLatLng(u, v); return new window.kakao.maps.LatLng(lat, lng); }); const alertCell = new window.kakao.maps.Polygon({ map: kakaoMap, path, strokeWeight: 3, strokeColor: "#d93636", strokeOpacity: 1, fillColor: "#e74c4c", fillOpacity: 0.48 }); const centre = cell.localPath.reduce((sum, point) => ({ u: sum.u + point.u / 4, v: sum.v + point.v / 4 }), { u: 0, v: 0 }); const { lat, lng } = toLatLng(centre.u, centre.v); const range = new window.kakao.maps.Circle({ map: kakaoMap, center: new window.kakao.maps.LatLng(lat, lng), radius: 35, strokeWeight: 2, strokeColor: "#d93636", strokeOpacity: 0.9, strokeStyle: "shortdash", fillColor: "#e74c4c", fillOpacity: 0.1 }); searchOverlays.push(alertCell, range); }); }
function locateMe() { const button = document.querySelector("#locateMe"); if (!navigator.geolocation) { setNotice("이 기기에서는 GPS 위치 기능을 사용할 수 없어요."); return; } button.disabled = true; button.textContent = "현재 위치를 확인하고 있어요…"; navigator.geolocation.getCurrentPosition(({ coords }) => { const { latitude: lat, longitude: lng, accuracy } = coords; const position = new window.kakao.maps.LatLng(lat, lng); if (myLocationMarker) myLocationMarker.setMap(null); myLocationMarker = new window.kakao.maps.Marker({ map: kakaoMap, position, title: "내 현재 위치" }); kakaoMap.panTo(position); const match = findGeoCell(lat, lng); if (match) { selectGeoCell(match.address, match.cell.polygon); setNotice(`내 위치는 ${match.address} 구역이에요. GPS 정확도는 약 ${Math.round(accuracy)}m예요.`); } else setNotice(`현재 위치는 해변 격자 범위 밖이에요. GPS 정확도는 약 ${Math.round(accuracy)}m예요.`); button.disabled = false; button.innerHTML = "내 위치로 격자 찾기 <span>⌖</span>"; }, () => { button.disabled = false; button.innerHTML = "내 위치로 격자 찾기 <span>⌖</span>"; setNotice("위치 권한이 필요해요. 브라우저에서 위치 사용을 허용해 주세요."); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }); }
...
function weatherInfo(code) { const values = { 0:["☀️","맑음"],1:["🌤️","대체로 맑음"],2:["⛅","구름 조금"],3:["☁️","흐림"],45:["🌫️","안개"],48:["🌫️","안개"],51:["🌦️","이슬비"],53:["🌦️","이슬비"],55:["🌦️","강한 이슬비"],61:["🌧️","비"],63:["🌧️","비"],65:["🌧️","강한 비"],71:["🌨️","눈"],73:["🌨️","눈"],75:["🌨️","강한 눈"],80:["🌦️","소나기"],81:["🌧️","소나기"],82:["⛈️","강한 소나기"],95:["⛈️","뇌우"],96:["⛈️","우박 동반 뇌우"],99:["⛈️","강한 뇌우"] }; return values[code] || ["🌡️", "날씨 상태 확인"]; }
async function loadWeather() { const beach = BEACHES[currentBeachKey] || BEACHES.haeundae, $ = (selector) => document.querySelector(selector); $("#weatherBeach").textContent = `${beach.name} 오늘의 날씨`; $("#weatherIcon").textContent = "…"; $("#weatherTemp").textContent = "불러오는 중…"; $("#weatherDesc").textContent = "현재 조건을 확인하고 있어요."; $("#weatherRain").textContent = "–"; $("#weatherWind").textContent = "–"; $("#weatherUpdated").textContent = ""; try { const params = new URLSearchParams({ latitude: beach.lat, longitude: beach.lng, current: "temperature_2m,apparent_temperature,precipitation,weather_code,wind_speed_10m", timezone: "Asia/Seoul" }); const response = await fetch(`https://api.open-meteo.com/v1/forecast?${params}`); if (!response.ok) throw new Error(); const data = await response.json(), now = data.current; if (!now) throw new Error(); const [emoji, label] = weatherInfo(now.weather_code); $("#weatherIcon").textContent = emoji; $("#weatherTemp").textContent = `${Math.round(now.temperature_2m)}°C`; $("#weatherDesc").textContent = `${label} · 체감 ${Math.round(now.apparent_temperature)}°C`; $("#weatherRain").textContent = `${now.precipitation} mm`; $("#weatherWind").textContent = `${Math.round(now.wind_speed_10m)} km/h`; $("#weatherUpdated").textContent = `기준 시각 ${now.time.replace("T", " ")} · 자동 갱신`; } catch { $("#weatherIcon").textContent = "⚠️"; $("#weatherTemp").textContent = "날씨 확인 필요"; $("#weatherDesc").textContent = "날씨 정보를 불러오지 못했어요."; $("#weatherUpdated").textContent = "네트워크를 확인한 뒤 다시 열어 주세요."; } }
function tideKind(value) { const text = String(value || ""); if (text === "1" || text === "3") return "만조"; if (text === "2" || text === "4") return "간조"; if (/고|high/i.test(text)) return "만조"; if (/저|low/i.test(text)) return "간조"; return "조석"; }
function tideTime(item) { const value = item?.predcDt || item?.tphTime || item?.tideTime || item?.fcstTime || item?.time || "", match = String(value).match(/(\d{2}:?\d{2})$/); return match ? match[1].replace(/(\d{2})(\d{2})$/, "$1:$2") : String(value || "시간 확인 중"); }
function tideHeight(item) { const value = item?.predcTdlvVl ?? item?.tphLevel ?? item?.tideLevel ?? item?.fcstLevel ?? item?.level; return value === undefined || value === null || value === "" ? "" : ` ${value}cm`; }
